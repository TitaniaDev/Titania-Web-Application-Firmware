<?php
    ...
    class HtmlDom {
        function FromURL($url = "") {
            if($url === "" || !is_string($url)) throw new FirmwareException("Expected string, got " . gettype($url), 55);
            return file_get_html($url);
        }
        function FromHTML($html = "") {
            if($html === "" || !is_string($html)) throw new FirmwareException("Expected string, got " . gettype($html), 55);
    ...
    // Thanks @http://slopjong.de/2012/03/31/curl-follow-locations-with-safe_mode-enabled-or-open_basedir-set/ for
    // the curl_exec_follow function (^.^)
    ...
    // curl_exec_follow_nr (custom, nr = no request) returns a curl resource instead of executing it and returning the data

    function curl_exec_follow_nr($ch, $timeout = 10, $agent = "Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/38.0.2125.111 Safari/537.36", &$maxredirect = null) {
      // we emulate a browser here since some websites detect
      // us as a bot and don't let us do our job
      $user_agent = "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.7.5)".
                    " Gecko/20041107 Firefox/1.0";
    ...
    function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
    {
        ...
        // REMOVED file_get_contents dependency, integrated CURL
        // file_get_contents does not work on all servers
